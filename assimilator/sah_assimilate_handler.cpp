        ...
        if (!result_file.eof()) {
            ...
            if (max_signals_allowed == 0 || signal_count <= max_signals_allowed) {
                ...
                if (signal_id) {
                    ...
                    log_messages.printf(SCHED_MSG_LOG::MSG_CRITICAL,
                                "[%s] Could not insert %s for sah result %"INT8_FMT". SQLCODE is %d. q_pix is %"INT8_FMT"  ra is %lf  decl is %lf  Exiting.\n",
...
int pre_process(T& signal, receiver_config& receiver_cfg, sqlint8_t sah_result_id, char * wu_name, bool check_rfi) {
    ...
    if(check_rfi) {
        common_signal.populate(signal);
        if (is_rfi(common_signal)) {
            log_messages.printf(SCHED_MSG_LOG::MSG_DEBUG,
        ...
        signal.rfi_checked = common_signal.rfi_checked;
        signal.rfi_found   = common_signal.rfi_found;
    }
